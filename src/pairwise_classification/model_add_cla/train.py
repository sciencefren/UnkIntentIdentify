...
def train(dataset):
    with tf.Graph().as_default():
        ...
        with sess.as_default():
            ...
            pwc_ckpt_path = os.path.join('../../../UnkIntentIdentifyModel/pwc_add_cla_ckpt', 'model.ckpt')
            ...
            while True:
                ...
                _, loss_cla, loss_supervised, loss_unsupervised, step = sess.run([model.train_op,
                    model.loss_cla,
                    model.loss_supervised,
                    model.loss_unsupervised,
                    model.global_step],
                ...
                if not (lambda_stable_steps+1)%print_info_steps:
                    print("[PWC] step: {}, loss_cla: {:.4}, loss_supervised: {:.4}, loss_unsupervised: {:.4}, bound delta: {:.4}".format(step,
                        loss_cla,
                        loss_supervised,
                        loss_unsupervised,
                        bound_delta))
